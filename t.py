...
def start(update: Update, context: CallbackContext) -> None:
    keyboard = [
        [InlineKeyboardButton("Check Date", callback_data='check_date')]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text('Merhaba! Lütfen bir seçenek seçin:', reply_markup=reply_markup)

def button(update: Update, context: CallbackContext) -> None:
    query = update.callback_query
    query.answer()
    if query.data == 'check_date':
        try:
            ...
            query.edit_message_text(text=f"Hata: {e}")
...
def main() -> None:
    ...
    # Komutları ve callback query handler'ı ekleyin
